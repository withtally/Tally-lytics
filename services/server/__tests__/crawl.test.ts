...
jest.doMock('../../crawling/crawlerManager');
jest.doMock('../../logging');
...
describe('Crawl API Routes', () => {
  ...
  const makeRequest = async (path: string, method: string = 'GET', headers: Record<string, string> = {}) => {
  ...
  describe('GET /api/crawl/status', () => {
    it('should return all crawler statuses successfully', async () => {
      ...
      mockCrawlerManager.getAllStatuses.mockReturnValue(mockStatuses);
    ...
    it('should handle crawler manager errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to get crawler statuses', { error: expect.any(Error) });
    });
  ...
  describe('GET /api/crawl/status/:forumName', () => {
    it('should return specific forum status', async () => {
      // Given
      const mockStatus = { forumName: 'ARBITRUM', status: 'idle', lastRun: '2024-01-01T00:00:00.000Z' };
      mockCrawlerManager.getStatus.mockReturnValue(mockStatus);
    ...
    it('should return 404 when forum not found', async () => {
      // Given
      mockCrawlerManager.getStatus.mockReturnValue(null);
    ...
    it('should handle internal errors', async () => {
      ...
      expect(mockLogger.error).toHaveBeenCalledWith('Failed to get status for forum', { error: expect.any(Error) });
    });
  });
  ...
  describe('POST /api/crawl/start/all', () => {
    it('should start crawls for all forums successfully', async () => {
      // Given
      mockCrawlerManager.getAllStatuses.mockReturnValue([]);
      mockCrawlerManager.startCrawl.mockResolvedValue(undefined);
      ...
      expect(mockLogger.info).toHaveBeenCalledWith('[CRON SERVICE] Received request to start all crawls', {
        source: 'Test Agent',
        ip: '127.0.0.1',
        timestamp: '2024-01-01T00:00:00.000Z',
      });
  ...
  describe('POST /api/crawl/start/:forumName', () => {
    ...
    it('should handle crawler start errors', async () => {
      ...
      mockCrawlerManager.startCrawl.mockRejectedValue(new Error('Crawl initialization failed'));
  ...
  describe('POST /api/crawl/stop/:forumName', () => {
    it('should stop crawl for specific forum successfully', async () => {
      ...
      mockCrawlerManager.stopCrawl.mockResolvedValue(undefined);
      mockCrawlerManager.getStatus.mockReturnValue(mockStatus);
    ...
    it('should handle errors during crawl stop', async () => {
      // Given
      mockCrawlerManager.stopCrawl.mockRejectedValue(new Error('Failed to stop crawler'));
    ...
    it('should handle non-Error exceptions', async () => {
      // Given
      mockCrawlerManager.stopCrawl.mockRejectedValue('String error');
