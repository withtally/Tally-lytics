  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      if (recordsArray.length > 0) {
        const uniqueForums = [
          ...new Set(recordsArray.map(item => item.forum_name).filter(Boolean)),
        ];
            ...
            <div className="grid grid-cols-3 gap-4 mb-2">
              <div>
                ...
                <p className="text-sm font-medium">
                  {post.post_evaluation.quality_score.toFixed(1)}/10
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Relevance</span>
                <p className="text-sm font-medium">
                  {post.post_evaluation.relevance_score.toFixed(1)}/10
                </p>
              </div>
              <div>
          ...
          <div className="border-t pt-3">
            ...
            <div className="grid grid-cols-2 gap-4 mb-2">
              <div>
                <span className="text-xs text-gray-500">Quality</span>
                <p className="text-sm font-medium">
                  {topic.topic_evaluation.quality_score.toFixed(1)}/10
                </p>
              </div>
              <div>
                <span className="text-xs text-gray-500">Relevance</span>
                <p className="text-sm font-medium">
                  {topic.topic_evaluation.relevance_score.toFixed(1)}/10
                </p>
              </div>
            </div>
        ...
        {isConnected === false && (
          <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-4 py-3 rounded mb-6">
            <div className="flex items-center">
              ...
              <span>
                Backend server is not connected. Please ensure the server is running on port 3004.
              </span>
            </div>
          </div>
        )}
            ...
            <>
              {activeTab === 'posts'
                ? (records as Post[]).map((post, index) => (
                    <div key={`post-${post.id}-${index}`}>{renderPost(post)}</div>
                  ))
                : (records as Topic[]).map((topic, index) => (
                    <div key={`topic-${topic.id}-${index}`}>{renderTopic(topic)}</div>
                  ))}
            </>
          )}
