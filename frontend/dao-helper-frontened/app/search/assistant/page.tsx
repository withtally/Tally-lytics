...
export default function AIAssistantPage() {
  ...
  const [chatHistory, setChatHistory] = useState<{role: string, content: string, timestamp: string}[]>([
    {
      role: 'assistant',
      content: 'Hello! I\'m your DAO research assistant. I can help you analyze DAO governance proposals, summarize discussions, find information, and more. How can I assist you today?',
  ...
  const sendQuestion = () => {
    ...
    setTimeout(() => {
      ...
      if (userQuestion.toLowerCase().includes('uniswap')) {
        aiResponse = "Based on my analysis of Uniswap's governance forums, there are currently 3 active proposals:\n\n" +
      ...
      } else if (userQuestion.toLowerCase().includes('makerdao') || userQuestion.toLowerCase().includes('treasury')) {
        aiResponse = "MakerDAO's treasury management strategy has evolved significantly in the past year. They currently manage approximately $8.3B in assets with the following allocation:\n\n" +
      ...
      } else if (userQuestion.toLowerCase().includes('aave') || userQuestion.toLowerCase().includes('risk')) {
        aiResponse = "Aave's risk management framework consists of three primary components:\n\n" +
      ...
      } else if (userQuestion.toLowerCase().includes('trend') || userQuestion.toLowerCase().includes('compare')) {
        aiResponse = "Based on my analysis of governance data across major DAOs, I've identified these key trends over the past quarter:\n\n" +
